...
num_label_train_image = 300
num_unlabel_train_image = 200
num_validate_image = 1
num_testing_image = 2
image_width = 128
image_height = 128
num_channels = 8
...
train_wkt = pd.read_csv(os.path.join(data_path, 'new_AOI_3_Paris_Train_Building_Solutions.csv'))
...
def polygons2mask_layer(height, width, polygons, image_id):
    ...
    img_mask = np.zeros((height, width), np.int32)
...
def cache_train_16():
    ...
    testing_image_list = image_list[
                          num_unlabel_train_image+num_label_train_image+num_validate_image:num_unlabel_train_image+num_label_train_image+num_validate_image+num_testing_image]
    ...
    f_testing = h5py.File(os.path.join(data_path, 'testing.h5'), 'w', compression='blosc:lz4', compression_opts=9)
    ...
    imgs_test = f_testing.create_dataset('test', (num_testing_image, image_width, image_height, num_channels),
                                  dtype=np.float16)
    imgs_test_mask = f_testing.create_dataset('test_mask',
                                       (num_testing_image, image_width, image_height, num_mask_channels),
                                       dtype=np.uint8)
    ...
    validation_ids=[]
    test_ids = []
    tif_fname = os.path.join(data_path, 'MUL', 'MUL_{}.tif')
    #tif_fname = os.path.join(data_path, 'MUL/MUL_AOI_3_Paris_{}.tif')
    i = 0
    for image_id in tqdm(label_image_list):
        image = tiff.imread(tif_fname.format(image_id))
        height, width,_ = image.shape
        imgs[i] = image[:image_width, :image_height]
        imgs_mask[i] = generate_mask(image_id,width,
                                            height,
    ...
    for image_id in tqdm(unlabel_image_list):
        image = tiff.imread(tif_fname.format(image_id))
        height, width,_ = image.shape
        imgs_unlabel[i] = image[:image_width, :image_height]
    ...
    for image_id in tqdm(validate_image_list):
        image = tiff.imread(tif_fname.format(image_id))
        height, width, _ = image.shape
        imgs_validation[i] = image[:image_width, :image_height]
    ...
    i = 0
    for image_id in tqdm(testing_image_list):
        image = tiff.imread(tif_fname.format(image_id))
        height, width, _ = image.shape
        imgs_test[i] = image[:image_width, :image_height]
        imgs_test_mask[i] = generate_mask(image_id,
                                                     height,
                                                     width,
                                                     num_mask_channels=num_mask_channels,
                                                     train=train_wkt)[:image_width, :image_height,:]
        test_ids += [image_id]
        i += 1
    f_testing['test_ids'] = np.array(test_ids).astype('|S9')
    f_testing.close()

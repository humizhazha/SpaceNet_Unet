...
def preprocess_dynamic_lab(dir,num_classes, extraction_step,patch_shape,num_images_training, type,
    ...
    f = h5py.File(os.path.join(F.data_directory, 'testing.h5'), 'r')
    test_vols = np.array(f['test'])[:,:,:, 2]
    label = np.array(f['test_mask'])[:,:,:, type]

    x,y=get_patches_lab(test_vols,label,extraction_step,patch_shape,num_images_training=num_images_testing)
...
def test(patch_shape,extraction_step):
  ...
  with tf.Graph().as_default():
    ...
    with tf.Session() as sess:
      ...
      images_pred = recompose2D_overlap(predictions_test,128, 128, extraction_step[0],
                                                        extraction_step[1])
      ...
      # Evaluation
      pred2d = np.reshape(images_pred, (images_pred.shape[0] * 128 * 128))
      lab2d = np.reshape(labels_test, (labels_test.shape[0] * 128 * 128))
      sum = 0
      for i in range(F.number_test_images):
